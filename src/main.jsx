...
function placeAll() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,#e0f2fe,transparent_28%),radial-gradient(circle_at_top_right,#fce7f3,transparent_26%),#f8fafc]">
      ...
      <main className="mx-auto grid max-w-[1800px] grid-cols-1 gap-4 p-5 lg:grid-cols-[360px_1fr]">
        <aside className="rounded-[2rem] border border-white bg-white/85 p-4 shadow-xl shadow-slate-200/70 lg:sticky lg:top-32 lg:h-[calc(100vh-150px)]">
          ...
          <div className="thin-scroll grid max-h-[calc(100vh-270px)] grid-cols-1 gap-2 overflow-auto pr-1 sm:grid-cols-2 lg:grid-cols-1">
            {leftDrugs.map(drug => (
              <DrugCard
                key={drug.id}
                drug={drug}
